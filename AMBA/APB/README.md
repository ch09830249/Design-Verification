...
* 類比：在 APB 匯流排裡面，有且**只有一個主機可以連接到APB匯流排接口 (VIP)**，其他的只能連接從機（外設），所以 APB 匯流排只支援單主機模式
## 概念4：總線協定（Bus Protocol）
* 在這條水果運輸總線裡面，水果店依照自己的需求，發動訂單，廠商提供水果，水果店確認收到。這一個完整的流程以及順序被稱為水果總線在傳輸水果的協議。
* 類比：在 AP B匯流排裡面，主機先發起要存取的外設位址，外設依照要求傳回數據，主機收到數據，這一個完整的流程稱為 APB 匯流排協定。
## 小結
...
* 在這條水果運輸總線裡面，水果店依照自己的需求,使用大喇叭發出進貨還是退貨（命令）、哪個廠家（地址）、退貨的水果或進貨的水果（數據）
...
**Q1 :是否可以不需要APB_interconnect，APB_Interconect的作用是什麼？**
...
但當 CPU 需要連接多個外設的時候（CPU只管發出位址和指令），依照 CPU 位址範圍分配表：
