...
vlog -work ./log1/work +incdir+./tb ./tb/top_tb.sv
...
& vsim -sv_seed $seed -lib ./log1/work top_tb `
      -wlf ./log1/wave.wlf `
      -do "view wave; add wave -recursive sim:/top_tb; run -all" `	# 1. 自動開啟波形視窗 2. 將波形加到視窗中 3.執行模擬直到結束
      > ./log1/transcript 2>&1
