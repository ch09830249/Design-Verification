...
task bus_driver::drive_one_pkt(bus_transaction tr);
  ...
  @(posedge vif.clk);
  vif.bus_cmd_valid <= 1'b0;    // 一個 clk 解除 cmd valid
  vif.bus_op <= 1'b0;
  vif.bus_addr <= 16'b0;
  ...
  if(tr.bus_op == BUS_RD) begin
    tr.rd_data = vif.bus_rd_data;    // 如果是讀取操作，這裡直接將讀到的資料賦值給 rd_data
  end
...
```  
@p_sequencer.config_object.rd_reg_event;    // virtual sequence 中則要等待這個事件的到來
```
...
* **在沒有暫存器模型之前**
  只能啟動 sequence 透過前門（FRONTDOOR）存取的方式來讀取暫存器，局限較大，在 Reference Model（或其他 component）中難以控制
* **有了暫存器模型之後**
...
* **常用操作及其對期望值和鏡像值的影響**
read&write  操作：這兩個操作在前面已經使用過了。無論透過後門存取或前門存取的方式從 DUT 中讀取或寫入暫存器的值，在操作完成後，暫存器模型都會根據讀寫的結果更新期望值和鏡像值（二者相等）。  
peek&poke   操作：前文中也講述過這兩個操作的範例。在操作完成後，暫存器模型會根據操作的結果更新期望值和鏡像值（二者相等）。  
get&set     操作：set 操作會更新期望值，但是鏡像值不會改變。 get 操作會傳回暫存器模型中目前暫存器的期望值。  
update      操作：這個操作會檢查暫存器的期望值和鏡像值是否一致，如果不一致，那麼就會將期望值寫入DUT中，並且更新鏡像值，使其與期望值一致。每個由 uvm_reg 派生來的類別都會有 update 操作，其使用方式在上一節中已經介紹過。每個由 
uvm_reg_block 派生來的類別也有 update 操作，它會遞歸地呼叫所有加入此 reg_block 的暫存器的 update 任務。  
