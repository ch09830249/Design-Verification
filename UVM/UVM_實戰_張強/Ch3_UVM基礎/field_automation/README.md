...
```
* 上述幾個巨集分別用於要註冊的欄位是**整數、實數、枚舉型別、直接或間接衍生自 uvm_object 的型別、事件及字串型別**
...
parameter UVM_NOPACK = (1<<9);  // 該欄位不打包
```
* 剩餘的 7 bit 另有它用，這裡不做討論
...
endclass
```
**Q:** 對於多出來的這個字段，是不是也應該用 uvm_field_int 巨集來註冊呢？如果不使用巨集註冊的話，那麼當呼叫 print 函數時，在顯示結果中就看不到其值，但是如果使用了宏，結果就是這個根本就不需要在 pack 和 unpack 操作中出現的欄位出現了 (該欄位要 print 但不打包)
