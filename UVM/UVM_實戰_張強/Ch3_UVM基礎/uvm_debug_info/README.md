...
* **設定某個特定 component 的預設詳細程度閾值**
  * 在 base_test 中將 driver 的詳細程度閾值設定為 UVM_HIGH（UVM_LOW、UVM_MEDIUM、UVM_HIGH 的資訊都會被印出）Code 為：
```
virtual function void connect_phase(uvm_phase phase);
...
```
* 如果需要牽扯到**階層參照 (Hierarchical Reference) (就是上面例子的路徑)**，所以**需要在 connect_phase 及以後的 phase 才能呼叫這個函數 (這樣各個 component 才都已經實例化)**  
...
如果要把 driver 中所有的 UVM_WARNING 顯示為 UVM_ERROR
```
virtual function void connect_phase(uvm_phase phase);
